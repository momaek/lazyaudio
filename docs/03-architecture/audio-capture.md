...
- **麦克风输入设备可选**（v0.1 暂用默认设备，纳入规划待排期）：当前 §2.2 直接 `getUserMedia({audio:true})` 取系统默认麦克风。可加设备选择——技术上用 `enumerateDevices()` 列 `audioinput` + 录音时 `deviceId: { exact }` 约束即可，不写 native。动因之一：戴蓝牙耳机时若麦克风也走蓝牙，A2DP 会降级到 HFP（系统音/音乐变糊、变小、单声道），给用户手动把麦克风指到内置/有线设备的入口能缓解（彻底避坑仍靠"系统音走 SCKit 不走蓝牙麦"）。落地点：settings.md Tab 2「录音」加"输入设备"下拉；与 §8.0 设备热切换状态机复用同一套设备管理。
