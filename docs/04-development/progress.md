...
> **🔄 feat/T53-cloud-transcribe(PR #43)**:云端转录(OpenAI 兼容 Audio API)代码完成 —— 编排器按 privacyMode 路由 Pass B、云端模式禁 Pass A(F4.9)、settings.cloud.transcribeModel + 设置页本地/云端切换接 privacyMode + 转录模型输入;按「远端处理任意大小」直传 WAV 不引 ffmpeg;7 单测 + 全量 89 过/typecheck/lint 净;**待真机手测 AC(需真云端 key)**
