...
> **当前焦点**：M5 接近收尾。T50-T57 全部 ✅(T53/T54/T55/T56/T57 本轮真机手测过)。手测中发现并修了 3 处 bug(转录面板不自动滚 + 设置 partial 更新被默认值覆盖 + 设置卡片裁行),在 **PR #46**(待合)。**M5 只剩 T58 深色模式 toggle + 过渡**(🔲,依赖 T18 已满足)。跨里程碑遗留:**T15a 崩溃恢复**(🔄 需 SIGKILL 手测)、**spike-012 1h 压测**(硬件受限,仅 M2)、**T41 Pass A 渐进精修**(新候选,已写进 dev-plan)。
