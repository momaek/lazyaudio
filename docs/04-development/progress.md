...
> **当前焦点**：M5 收尾中。T50-T57 全部 ✅(T53-T57 真机手测过;手测中修的 3 处 bug 已随 PR #46 合并)。**🔄 T58 深色模式**(feat/T58-dark-mode):全 app 主题(nativeTheme.themeSource + 各窗口 initTheme)+ 150ms 切换过渡,代码就绪 typecheck/test 102/lint 净,**待真机手测**(四窗 visual review + 无闪烁)。注:M5 progress 对账(T50/T52/T53-T57→✅)原在 PR #46 的 commit 1598474,但 #46 在该 commit 推上去前就被合了,对账丢了 → 本 PR 一并带回。跨里程碑遗留:**T15a 崩溃恢复**(🔄 需 SIGKILL 手测)、**spike-012 1h 压测**(硬件受限,仅 M2)、**T41 Pass A 渐进精修**(候选)。
...
| T61 | 性能优化           | 🔲 todo | —   | —   | 按 dogfood 反馈；专项 spec 见 development-plan.md §6.2。**评测基建已先落**(feat/T61-eval-infra):`scripts/eval-transcribe-fixtures.ts`(Pass B CER/术语命中/RTF/RSS,复用 workers/asr 识别链;支持 m4a 等经 ffmpeg 解码 + srt 当参考稿 + `--dump-hyp`)+ `fixtures/transcribe/` 约定 + Pass A latency/RTF 埋点(`passa-metrics.ts`,env `LAZY_PASSA_METRICS` 开关,默认 no-op)。**首个基线**(dogfood 5 段共 171min 中文视频,含 2 圆桌多人):平均 CER 17.5% / RTF 0.017 / RSS 峰 1984MB。逐段:kunyuan_cpo 6.4% / sanxingdui(圆桌)14.7% / kunlun(圆桌)15.0% / wizard_lisa 19.9% / test 31.5%。关键发现:**deletion 几乎线性决定 CER**——3 段纯中文只丢 ~1%(替换/同音为主),仅 test(-23%)/lisa(-8%)中英专有名词密集段大量删除;专有名词跨段稳定错(巫师/Lisa/Slay/Trump → P1 术语表);圆桌多人扛得住(~15%@丢1%);RSS 1984MB(53min 整文件读内存)是 T40 内存监控信号。**尺子校准**(feat/T61-passa-eval):视频字幕参考稿是精修非逐字(书面数字 + 删语气词),eval 默认加对称归一(数字归一 `--no-norm-num` 关 / 语气词剥除 `--keep-fillers` 关),挤掉 ~2.1 点假错 → **校准基线 15.4%**(kunyuan 4.4% 引擎够好 / 圆桌 ~13.5% 多人上限 / lisa 17.6% 英文名 / trump 28.2% 真漏转待挖)。同支新增 **`--pass-a` 实时模拟评测**(切块喂真实 VadStream,confirmed 算 CER):Pass A CER 21.4% vs Pass B 17.5%(同口径,符合「实时草稿+离线精修」设计);⚠️ Pass A eval 中 RSS 峰 4477MB + RTF 随样本递增,疑 sherpa OfflineStream 每次 createStream 不释放,**长录音泄漏风险待查**(生产 utility 每录音一进程,暴露窗口=单次录音时长)。Pass B VAD 分段实验被 A/B 否决(PR #49,CER +0.9)。**流式 zh-zipformer + 热词 spike 否决**(spike/online-zh-hotwords):换 transducer(支持热词)即便 beam 仍输 SenseVoice(16.2 vs 15.4),纯中文也没赢;且热词对目标词(巫师/昆仑/三星堆,char 不在词表走 byte-fallback)无法编码→失效。结论:留 SenseVoice + 后处理术语表,详见 tech-feasibility「流式 zh-zipformer + 热词」。优化本体仍待 M6 dogfood 触发 |
