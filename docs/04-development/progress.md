...
| T61 | 性能优化           | 🔲 todo | —   | —   | 按 dogfood 反馈；专项 spec 见 development-plan.md §6.2。**评测基建已先落**(feat/T61-eval-infra):`scripts/eval-transcribe-fixtures.ts`(Pass B CER/术语命中/RTF/RSS,复用 workers/asr 识别链;支持 m4a 等经 ffmpeg 解码 + srt 当参考稿 + `--dump-hyp`)+ `fixtures/transcribe/` 约定 + Pass A latency/RTF 埋点(`passa-metrics.ts`,env `LAZY_PASSA_METRICS` 开关,默认 no-op)。**首个基线**(dogfood 5 段共 171min 中文视频,含 2 圆桌多人):平均 CER 17.5% / RTF 0.017 / RSS 峰 1984MB。逐段:kunyuan_cpo 6.4% / sanxingdui(圆桌)14.7% / kunlun(圆桌)15.0% / wizard_lisa 19.9% / wizard_trump 31.5%。关键发现:**deletion 几乎线性决定 CER**——3 段纯中文只丢 ~1%(替换/同音为主),仅 wizard_trump(-23%)/lisa(-8%)中英专有名词密集段大量删除;专有名词跨段稳定错(巫师/Lisa/Slay/Trump → P1 术语表);圆桌多人扛得住(~15%@丢1%);RSS 1984MB(53min 整文件读内存)是 T40 内存监控信号。**P0 #1 Pass B VAD 分段已试 → ❌ 否决**(feat/T61-passb-vad-seg;A/B 平均 CER 17.5%→18.4%,圆桌 +1.4/+2.2;漏转是模型层面非边界切的、VAD 又把多人轮替切碎伤上下文;详见 tech-feasibility「Pass B VAD 分段实验」)。生产保持定窗,VAD 实现保留作将来调参 infra。优化本体仍待 M6 dogfood 触发 |
