...
> **🔄 feat/T54-export**:导出 md/txt/srt 代码完成 —— export 域(IPC + 纯函数生成器 + save dialog 落盘)+ 详情头临时导出按钮(T55 ⋯ 菜单接管);10 单测(SRT 精确字节+md/txt 结构)+ 全量 99 过/typecheck/lint 净;**待真机手测**(dialog 落盘 + 打开正常)
