...
- [ ] T61 Pass A OfflineStream 内存泄漏修复（feat/T61-passa-leak-fix）
  - [x] 根因坐实 + 写入 tech-feasibility：sherpa OfflineStream `Napi::External` 用 `default_basic_finalizer`,从不调 `SherpaOnnxDestroyOfflineStream`；GC 证伪、升级 1.13.3 无效；验证：`scripts/repro-stream-leak.ts`（0.48MB/stream 线性、GC 压不下去）
  - [x] eval 尺子每 fixture 起子进程，隔离跨文件泄漏 → 干净 per-sample RTF/RSS；验证：`pnpm tsx scripts/eval-transcribe-fixtures.ts`（lisa 909MB / trump 618MB 各自独立,不再累积；CER 17.6/28.2 与校准基线吻合）
  - [x] 生产 Pass A worker 段边界进程回收（RSS 阈值触发,从 confirmed 水位续段号/时间轴,PCM 环形缓冲重放）；验证：`pnpm typecheck` 净 + `pnpm vitest run` 110 passed（含新增 offset 续接测试）
  - [ ] 真机长录音验证：连续录 ≥1h，RSS 被回收压在阈值内、转录无丢段/时间轴断裂；验证：**待真机手测**
  - [ ] 给 sherpa-onnx-node 提上游 issue（External finalizer 不调 DestroyOfflineStream）
...
| T61 | 性能优化           | 🔄 wip  | 2026-06-08 | —   | 因 dogfood 发现中英混合实时转录不理想,先做 T61a(feat/T61-pass-a-tuning):首个 hypothesis 延后 ≥2s + VAD/短窗参数抽取 + raw tags/耗时安全 debug;待真机 A/B。**评测基建**(PR #48):`scripts/eval-transcribe-fixtures.ts`(CER/术语/RTF/RSS,复用 workers/asr 链;m4a 经 ffmpeg + srt 当 ref + `--dump-hyp`)+ `fixtures/transcribe/` + Pass A 埋点(`passa-metrics.ts`,env 开关默认 no-op)。**尺子校准**(PR #52,feat/T61-passa-eval):视频字幕参考稿是精修非逐字(书面数字 + 删语气词),CER 默认对称归一(数字归一 `--no-norm-num` 关 / 语气词剥除 `--keep-fillers` 关),挤掉 ~2.1 点假错 → **校准基线 15.4%**(kunyuan 4.4% 引擎够好 / 圆桌 ~13.5% 多人上限 / lisa 17.6% 英文名 / trump 28.2% 真漏转)。同支新增 **`--pass-a` 实时模拟**(切块喂真实 VadStream 算 confirmed CER):Pass A 21.4% vs Pass B 17.5%(同口径,符合「实时草稿+离线精修」)。**OfflineStream 原生内存泄漏已坐实**(feat/T61-passa-leak-fix):根因 = sherpa-onnx-node 把 OfflineStream 包成 `Napi::External` 但用 `default_basic_finalizer`,从不调 `SherpaOnnxDestroyOfflineStream` → 每 stream 漏 ~0.48MB,GC 回收不掉、升级 1.13.3 无效(见 tech-feasibility「OfflineStream 原生内存泄漏」+ `scripts/repro-stream-leak.ts`)。修:(a) eval 尺子每 fixture 起子进程隔离跨文件泄漏(per-sample RTF/RSS 干净);(b) 生产 Pass A worker 在段边界按 RSS 阈值(`LAZY_PASSA_RSS_LIMIT_MB` 默认 1500)进程回收,从上次 confirmed 水位续段号/时间轴(代码 + 单测就绪,**真机长录音 RSS 上限待验**);(c) 上游 issue 待提。**否决的路**:Pass B VAD 分段(PR #49,CER +0.9)/ 流式 zh-zipformer + 热词(spike/online-zh-hotwords,16.2 vs 15.4 且热词对 byte-fallback 字失效);均见 tech-feasibility。**P1 已落:字母空格合并**(feat/T61-terms-table):SenseVoice 把英文缩写输出成「c p o」,后处理合并成「CPO」(`collapseSpacedLetters`,Pass A/B 共用,≥2 连续单字母才合并)→ 用户视角术语命中 59.7%→74.2%(CER 不变,本就去空格);配 5 段 terms.json 量术语命中(本地)。真漏只剩 8/31 是英文 mangle(Trump→创/BEA→逼,错成常用字无法安全反映射,错→对词表咬不到)→ 术语表 reach 有限。优化本体仍待 M6 dogfood 触发 |
