...
| T61 | 性能优化           | 🔲 todo | —   | —   | 按 dogfood 反馈；专项 spec 见 development-plan.md §6.2。**评测基建已先落**(feat/T61-eval-infra):`scripts/eval-transcribe-fixtures.ts`(Pass B CER/术语命中/RTF/RSS,复用 workers/asr 识别链;支持 m4a 等经 ffmpeg 解码 + srt 当参考稿 + `--dump-hyp`)+ `fixtures/transcribe/` 约定 + Pass A latency/RTF 埋点(`passa-metrics.ts`,env `LAZY_PASSA_METRICS` 开关,默认 no-op)。**首个基线**(dogfood 2 段共 47min 中文视频):平均 CER 25.7%(test 14:52=31.5% / wizard_lisa 30:30=19.9%)/ RTF 0.017 / RSS 峰 1151MB。CER 差异主因 = Pass B 定窗 15s 边界吞字(test 少 23% 内容、lisa 少 8% → P0 #1 VAD 分段);专有名词跨段稳定错(巫师/Lisa/Slay/Trump → P1 术语表);开头首窗必坏。优化本体仍待 M6 dogfood 触发 |
