...
Electron 在 14.2+ 上走 CoreAudio Tap(仅麦克风权限)还是回退 ScreenCaptureKit(屏幕录制权限),取决于 Chromium feature `MacCatapLoopbackAudioForScreenShare` 是否默认启用 — **Electron 39+ 才默认开,35 会回退**(详见文末「实测订正」);14.1 及以下一律回落 ScreenCaptureKit,触发"屏幕录制"权限弹窗。
...
- `src/main/system/audio-sources.ts`:只走 `desktopCapturer` + Electron 39+ 默认 CoreAudio Tap 行为,不引入 SCStream 分支
...
## 实测订正(2026-05-29 / PR #30)

ADR 背景节原写"Electron 35+ 自动走 CoreAudio Tap" — **不准**。dev 手测(M2 / macOS 26.5 Tahoe / Electron 35.7.5):系统音 loopback 回退 ScreenCaptureKit,误索「屏幕录制」权限,与本 ADR 决策(只要麦克风权限)直接冲突。

根因:CoreAudio Tap loopback 由 Chromium feature `MacCatapLoopbackAudioForScreenShare` 控制,**Electron 39+ 才默认启用**(spike-005 用 Electron 42 验证;见 tech-feasibility §R1 工具链需求)。

处置:v0.1 Electron 基线锁 **≥ 42**(对齐 spike-005 验证环境),保证 catap 默认走、权限清单只有麦克风。决策本身(只走 CoreAudio Tap)不变,变的只是"哪个 Electron 版本能兑现它"。
