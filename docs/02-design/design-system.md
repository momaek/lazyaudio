...
/* UI */
font-family:
  -apple-system, BlinkMacSystemFont, 'SF Pro Text', 'Segoe UI', 'PingFang SC', 'Microsoft YaHei',
...
§2.1 的 gray 阶是浅色调色板。组件大量直接用 raw gray token 做文字色
（`color: var(--color-gray-900)` 等），深色下必须**反转**这套阶，否则暗字压暗底看不清。
不是纯对称反转——muted 档（400/500，文字用量最大）按可读性微调，保证在深色面上对比足够。

| token    | 浅色（§2.1） | 深色映射  | 主要角色               |
| -------- | ------------ | --------- | ---------------------- |
| gray-50  | #FAFAFA      | `#18181B` | 面                     |
| gray-100 | #F4F4F5      | `#27272A` | raised 面              |
| gray-200 | #E4E4E7      | `#3F3F46` | 边框                   |
| gray-300 | #D4D4D8      | `#52525B` | 分隔 / 边框 / 开关轨   |
| gray-400 | #A1A1AA      | `#8B8B94` | 更弱的 muted / icon    |
| gray-500 | #71717A      | `#A1A1AA` | muted 正文（用量最大） |
| gray-600 | #52525B      | `#C4C4CC` | —                      |
| gray-700 | #3F3F46      | `#D4D4D8` | 次级文字               |
| gray-800 | #27272A      | `#E4E4E7` | —                      |
| gray-900 | #18181B      | `#F4F4F5` | 主文字 → 近白          |
| gray-950 | #09090B      | `#FAFAFA` | 最深文字 → 白          |

- 落地：`src/renderer/styles/dark.css` 的 `.dark` 块。背景 / 边框另由 §2.4 的 `bg-l*` / `border` token 处理（已反转），高对比元素（如播放按钮 `gray-950` 底 + `bg-l1` 图标）会整体翻转、对比保持。
- 例外：prep 浮窗自带整套手写 `.dark` 规则（旧未反转语义），在 `prep.css` 里把 gray 阶钉回原值、不受全局反转影响。
