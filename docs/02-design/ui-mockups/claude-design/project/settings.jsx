...
function Toggle({ on }) { return <span className={'toggle' + (on ? ' is-on' : '')}/>; }
...
function Select({ value, mono, minWidth }) {
  return (
    <button className={'select-trigger' + (mono ? ' is-mono' : '')} style={minWidth ? { minWidth } : null}>
      <span>{value}</span>
...
function SettingsGeneral() {
  return (
    <>
      ...
      <h3 className="setting-group-title" style={{ marginTop: 8 }}>提醒</h3>
      <div className="setting-rows">
...
function ModelCard({ name, desc, lang, status, progress, downloaded, size, isDefault }) {
  return (
    <div className={'model-card' + (isDefault ? ' is-default' : '')}>
      ...
      {status === 'done' && (<div className="status is-done"><span>✓</span><span>已下载</span></div>)}
      {status === 'available' && (<div className="status is-avail"><span>↓</span><span>未下载</span></div>)}
      {status === 'downloading' && (
        <div className="status is-downloading">
          <div className="pbar"><div className="fill" style={{ width: progress + '%' }}/></div>
          <div className="meta"><span>{progress}%</span><span>{downloaded}</span></div>
        </div>
      )}
      <div className="right">
        ...
        <div className="actions">
          ...
          {status === 'done' && isDefault && (<button className="btn-compact-ghost danger">删除</button>)}
...
function SettingsTranscribeLocal() {
  return (
    <>
      <div className="set-page-head">
        <h2>转录引擎</h2>
        <div className="sub">选择把音频变成文字的方式。本地模型保证音频不出本机；云端 API 更快但会上传内容。</div>
      </div>
      ...
      <div className="model-list">
        <ModelCard lang="zh" name="whisper-medium-zh" desc="中文优先 · 多语种回退 · CTranslate2 量化" status="done" size="272 MB" isDefault/>
        <ModelCard lang="zh" name="whisper-small-zh"  desc="中文优先 · 比 medium 快 ~2× · 适合实时字幕" status="done" size="244 MB"/>
        <ModelCard lang="en" name="whisper-medium-en" desc="英文专用 · 准确率最高 · 用于英文会议 / 面试" status="downloading" progress={63} downloaded="171 / 272 MB" size="272 MB"/>
        <ModelCard lang="ml" name="whisper-large-v3"  desc="多语种 · 最高准确率 · 速度较慢 · 推荐 M 系列芯片" status="available" size="1.55 GB"/>
        <ModelCard lang="zh" name="whisper-tiny-zh"   desc="最小模型 · 适合低配设备做实时草稿" status="available" size="39 MB"/>
      </div>
    </>
  );
}
    ...
    <>
      ...
      <div className="cloud-form">
        <div className="form-row">
          ...
          <div className="ctl"><input className="input-lg is-mono" defaultValue="https://api.openai.com/v1" readOnly/></div>
        </div>
        <div className="form-row">
          <span className="lbl">apiKey</span>
          <div className="ctl">
            <input className="input-lg is-mono" type="password" defaultValue="sk-proj-abCdEf1234567890" readOnly/>
        ...
        <div className="form-row">
          ...
          <div className="ctl"><input className="input-lg is-mono" defaultValue="gpt-4o-mini-transcribe" readOnly/></div>
        </div>
        <div className="form-row">
          <span className="lbl">语言</span>
          <div className="ctl"><input className="input-lg" defaultValue="自动检测（推荐）" readOnly/></div>
        </div>
        <div className="test-row">
          <button className="btn btn-secondary">测试连接</button>
          <span className="status is-ok">
            <span>✓</span><span>已连接</span>
            <span className="mono">· 延迟 84ms · 余额 $12.40</span>
          </span>
        </div>
        <div className="save-row"><button className="btn btn-primary">保存</button></div>
      </div>
    </>
  );
}
...
function SettingsRecording() {
  ...
  return (
    <>
      ...
      <h3 className="setting-group-title" style={{ marginTop: 8 }}>存储</h3>
      <div className="setting-rows">
  ...
  return (
    <div className={'short-row' + (capturing ? ' is-capturing' : '')}>
        ...
        {capturing ? (
          ...
          keys.map((k, i) => (
            <React.Fragment key={i}>
              <span className={'kkey' + (k.length === 1 && /[^\w]/.test(k) ? ' is-symbol' : '')}>{k}</span>
      ...
      {capturing ? (
        ...
        <button className={'short-edit-link' + (danger ? ' muted' : '')}>{danger ? '清除' : '修改'}</button>
      )}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="short-top-row">
        <span className="setting-group-title" style={{ marginBottom: 0 }}>动作</span>
...
function SettingsPrivacy({ showModal }) {
  return (
    <>
      ...
      <h3 className="setting-group-title" style={{ marginTop: 8 }}>诊断</h3>
      <div className="setting-rows">
      ...
      <h3 className="setting-group-title" style={{ marginTop: 8 }}>危险操作</h3>
      <div className="danger-card">
        <div className="text">
          <div className="title-row">清除全部录音和转录</div>
          <div className="desc">将永久删除所有本地音频文件、转录文本和 AI 摘要。此操作不可撤销。</div>
        </div>
        <button className="btn-danger">清除全部数据…</button>
      </div>

      <div className="privacy-note">
        LazyAudio 不会向任何服务器发送音频、转录文本或使用数据。开启「云端 API」转录引擎时，音频片段会按你配置的 baseUrl 上传至该服务，请自行确认其隐私条款。
      </div>
      ...
      {showModal && (
        <div className="modal-scrim">
          <div className="modal-card">
            ...
            <h3>确认清除全部数据？</h3>
            <div className="body">
              将永久删除 <code>327 个录音</code>（共 <code>4.2 GB</code>）及其所有转录和摘要。<br/>
...
function SettingsWindow({ theme = 'light', tab = 'local', nav = 'transcribe', showModal }) {
  const content =
    ...
    nav === 'recording'  ? <SettingsRecording/>           :
    nav === 'transcribe' ? (tab === 'local' ? <SettingsTranscribeLocal/> : <SettingsTranscribeCloud/>) :
    nav === 'shortcut'   ? <SettingsShortcut/>            :
    nav === 'privacy'    ? <SettingsPrivacy showModal={showModal}/> :
    nav === 'about'      ? <SettingsAbout/>               :
