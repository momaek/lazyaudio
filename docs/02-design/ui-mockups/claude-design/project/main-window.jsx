    ...
    items: [
      {
        ...
        type: 'note',
        state: 'done',
        title: '关于 onboarding 卡顿的想法',
        timeOfDay: '16:45',
        dur: '00:01:23',
        preview: 'app 启动后第一屏到模型下载这段路径太长，应该把权限请求挪后',
      },
      {
        id: 's4',
        type: 'lecture',
        ...
        timeOfDay: '14:00',
        dur: '01:30:00',
        preview: '我们今天主要讲 Paxos 和 Raft，重点关注它们的 leader election 机制',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        timeOfDay: '周四 11:00',
        dur: '00:38:12',
        preview: '请你设计一个支持百万 QPS 的短链服务，可以从需求开始',
      },
      {
        id: 's6',
        type: 'meeting',
        state: 'done',
        title: '和 Ops 同步 — Q3 容量规划',
        timeOfDay: '周三 09:00',
        dur: '00:25:00',
        preview: '基于上半年的曲线，预估 Q3 集群需要扩容 30%，主要是…',
      },
      {
        id: 's7',
        type: 'general',
        ...
        timeOfDay: '周一 08:12',
        dur: '00:00:42',
        preview: '记一下今天通勤想到的，关于 retention 看板的二级跳转…',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        timeOfDay: '5/8',
        dur: '00:52:30',
        preview: '今天我们请到了…来聊一下他们这两年从 ECS 迁到 K8s 的过程…',
      },
      {
        id: 's9',
        type: 'interviewer',
        ...
        timeOfDay: '5/6',
        dur: '00:42:18',
        preview: '你简历上写了一个高并发缓存的项目，能展开讲讲你具体做了什么吗',
      },
    ],
  },
]
...
const TRANSCRIPT = [
  { t: '00:00', sp: '1', text: '好的我们今天主要聊一下你过去一年的工作，先简单做个自我介绍吧' },
  {
    t: '00:11',
    sp: '2',
    text: '好的，我叫陈昊，过去一年在一家 B 端 SaaS 公司做数据分析平台，主要负责后端架构',
  },
  ...
  { t: '01:32', sp: '1', text: '这个选型是你主导的吗' },
  {
    t: '01:38',
    sp: '2',
    text: '嗯是我主导的，最早我们用 MySQL 直接跑聚合，QPS 上来之后基本撑不住',
  },
  { t: '01:55', sp: '1', text: '那你当时怎么调研的，主要关注哪几个指标' },
  {
    t: '02:01',
    sp: '2',
    text: '我列了一个备选清单，看三件事：写入吞吐、查询延迟、运维复杂度。OLAP 我们对比了 Druid、Pinot、ClickHouse',
  },
...
function ListItem({ item, selected, recording }) {
  ...
  return (
    <div
      className={['lib-item', selected && 'is-selected', recording && 'is-recording']
        .filter(Boolean)
        .join(' ')}
      data-type={item.type}
    >
      ...
      <div className="lib-item-title">
        <b>{item.title}</b>
        {recording ? (
          <span className="rec-tag">（录制中）</span>
        ) : (
          <span className="timeofday">{item.timeOfDay}</span>
        )}
      </div>
...
function TypeChip({ type, label, active }) {
  return (
    <button className={'chip' + (active ? ' is-active' : '')} data-type={type}>
      {label}
    </button>
  )
}
...
function Library({ activeChip, selectedId, recordingId, empty }) {
  return (
    <aside className="lib">
      <button className="lib-tool-btn" title="设置">
        {Icons.gear(16)}
      </button>
      ...
      {empty ? (
        <div className="lib-list-empty">
          <div>这里会显示你的所有录音</div>
          <div>
            按下 <span className="kbd">⌘⇧R</span> 开始第一次录音
          </div>
        </div>
      ) : (
        <div className="lib-list">
          {recordingId && (
            <ListItem item={RECORDING_SESSION} selected={selectedId === recordingId} recording />
          )}
          {LIBRARY.map((group) => (
            <div key={group.group}>
  ...
  return (
    <section className="detail">
      <header className="dh">
        ...
        <span className="type-badge" data-type="interviewer">
          {Icons.glyph.interviewer()}面试官
        </span>
        ...
        <div className="dh-actions">
          <button className="icon-btn" title="重新转录">
            {Icons.refresh()}
          </button>
      ...
      <div className="pc">
        <button className="pc-play" title="播放">
          {Icons.play(11)}
        </button>
        <div className="pc-time">
          <span className="now">00:14</span>
          <span className="sep">/</span>
          <span className="total">01:12:34</span>
        </div>
    ...
    <div className="tx">
      <div className="tx-head">
        ...
        <div className="tx-head-right">
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <span
              style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--speaker-1)' }}
            />
            麦克风
          </span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <span
              style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--speaker-2)' }}
            />
            系统音
          </span>
        </div>
      ...
      <div className="tx-list">
        {TRANSCRIPT.map((seg, i) => (
          <div
            key={i}
            className={'tx-seg' + (i === CURRENT_SEG_IDX ? ' is-current' : '')}
            data-sp={seg.sp}
          >
            <span className="tx-seg-time">{seg.t}</span>
            <span className="tx-seg-speaker">
              <span className="sp-dot" />
              <span className="sp-letter">{seg.sp === '1' ? 'M' : 'S'}</span>
            </span>
  ...
  return (
    <div className="sm">
      <div className="sm-head">
        AI 摘要<span className="template">{Icons.template()}面试评估</span>
      </div>
      <div className="sm-body">
        <div className="sm-section">
          ...
          <ul>
            <li>
              5 年后端经验，过去一年在 B 端 SaaS 做数据分析平台，<b>负责 OLAP 架构选型与落地</b>
            </li>
        ...
        <div className="sm-section">
          ...
          <ul>
            <li>
              <span className="tag">01:38</span>能讲清楚 MySQL → ClickHouse 的迁移动机，关注 QPS
              上量后的真实瓶颈
            </li>
            <li>
              <span className="tag">02:01</span>调研流程结构化（吞吐 / 延迟 / 运维），不是凭直觉选型
            </li>
          </ul>
        </div>
        <div className="sm-section">
  ...
  return (
    <section className="detail">
      <header className="dh">
        ...
        <span className="type-badge" data-type="note">
          {Icons.glyph.note()}笔记
        </span>
        <span className="dh-meta">
      ...
      <div className="pc is-disabled">
        ...
        <div className="pc-time">
          ...
          <span className="sep">/</span>
          <span className="total">03:24</span>
        </div>
        <div className="pc-right">
          <button className="pc-btn">
            {Icons.skipBack()}
      ...
      <div className="tx-summary">
        <div className="tx">
          <div className="tx-head">
            转录
            <div className="tx-head-right" style={{ color: 'var(--text-subtle)' }}>
              录音结束后自动开始
            </div>
          </div>
        ...
        <div className="sm is-disabled">
          <div className="sm-head">
            AI 摘要
            <span className="template" style={{ color: 'var(--text-subtle)' }}>
              {Icons.template()}笔记模板
            </span>
          </div>
    ...
    <section className="detail">
      <div className="empty-stage">
        ...
        <div className="empty-tips">
          <div className="row">
            <span
              style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--text-subtle)' }}
            />
            录音会同时捕获麦克风和系统音
          </div>
          <div className="row">
  ...
  return (
    <section className="detail">
      <header className="dh">
        ...
        <span className="type-badge" data-type="meeting">
          {Icons.glyph.meeting()}会议
        </span>
        <span className="dh-meta">
          <span>00:52:18</span>
          <span className="dot-sep" />
          <span style={{ color: 'var(--warning)' }}>转录中</span>
          <span className="dot-sep" />
          <span>5 分钟前停止</span>
        </span>
        <div className="dh-actions">
      ...
      <div className="pc is-disabled">
        ...
        <div className="pc-time">
          ...
          <span className="sep">/</span>
          <span className="total">00:52:18</span>
        </div>
        <div className="pc-right">
          <button className="pc-btn">
            {Icons.skipBack()}
      ...
      <div className="tx-summary">
        <div className="tx">
          <div className="tx-progress">
            ...
            <button className="pc-btn" style={{ height: 24, padding: '0 8px', fontSize: 11 }}>
              取消
            </button>
          </div>
          <div className="tx-placeholder">
  ...
  return (
    <section className="detail">
      <header className="dh">
        ...
        <span className="type-badge" data-type="general">
          {Icons.glyph.general()}通用
        </span>
        <span className="dh-meta">
          <span>00:00:42</span>
          <span className="dot-sep" />
      ...
      <div className="pc">
        ...
        <div className="pc-time">
          <span className="now">00:00</span>
          <span className="sep">/</span>
          <span className="total">00:00:42</span>
        </div>
        <div className="pc-right">
          <button className="pc-btn">
            {Icons.skipBack()}
      ...
      <div className="tx-summary">
        <div className="tx">
          <div className="tx-head">
            转录
            <div className="tx-head-right" style={{ color: 'var(--danger)' }}>
              <span>⚠ 失败</span>
            </div>
          </div>
          <div className="error-stage">
            ...
            <div className="err-msg">
              转录引擎设置为「云端 API」，但请求{' '}
              <code style={{ fontFamily: 'var(--font-mono)' }}>api.openai.com</code>{' '}
              时超时。录音文件已完整保存，可重试或切换至本地引擎。
            </div>
