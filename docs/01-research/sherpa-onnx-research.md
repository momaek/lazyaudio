...
| ------------- | ----------------------------------------------------------------- | ----------------------------------- |
| 包            | **`sherpa-onnx-node`** + 平台二进制包                             | **`sherpa-onnx`**（捆绑 wasm 入口） |
...
> **2026-05-17 修订**(由 spike-011 实测发现):npm 上有两个容易混淆的包名 — `sherpa-onnx`(WASM,index.js 顶部就 `require('./sherpa-onnx-wasm-nodejs.js')`)和 **`sherpa-onnx-node`**(真正的 N-API addon)。本文档早先把 N-API 一栏写成了 `sherpa-onnx`,实测后已更正:**v0.1 应该装 `sherpa-onnx-node`,不是 `sherpa-onnx`**。

**结论：用 N-API 那套**（`sherpa-onnx-node` + 平台二进制包），WASM 仅在"我就想塞渲染进程里、Worker 里跑个 VAD"这种特殊场景才考虑。

---

## 4. NPM 包结构

`sherpa-onnx-node` 是入口包，运行时通过 `process.platform` + `process.arch` 动态加载对应的二进制包。需要在 `package.json` 把入口包和**当前平台**的二进制包都装上：

```
sherpa-onnx-node                 # 主包,纯 JS 入口 + N-API bindings(注意是 -node 后缀)
